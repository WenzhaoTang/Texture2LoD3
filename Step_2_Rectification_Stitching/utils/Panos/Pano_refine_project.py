...
def Refine(im, width, height, focal, params, tmp_count):
    ...
    # Assume hvp_groups is a list of numpy arrays, each containing indices.
    candidates["hvp_groups"] = []
    for group in hvp_groups:
        if isinstance(group, np.ndarray):  # Ensure it is an array of indices.
            for index in group:
                if index < len(helpfulIds):
                    candidates["hvp_groups"].append(helpfulIds[index])
