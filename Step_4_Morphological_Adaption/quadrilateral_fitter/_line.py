...
class _Line:
    ...
    def __copy__(self):
        return _Line(A=self.A, B=self.B, C=self.C, centroid=self.centroid)
    ...
    def move_line_outwards(self, distance):
        """
        Moves the line outward from the centroid by a specified distance.
        """
        norm = np.sqrt(self.A**2 + self.B**2)
        direction = np.sign(self.point_line_position(self.centroid.x, self.centroid.y))
        self.C += direction * distance * norm