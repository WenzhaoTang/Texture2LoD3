...
def do_raycast(mesh, cam_origin, angle_deg, pitch_deg=0):
    ...
    hits, _, index_tri = mesh.ray.intersects_location(
        ray_origins=origins,
...
def visualize_3d_scene(mesh, cam_origin, hits_list, out_path=None, title="3D Raycasting Demo"):
    ...
    for item in hits_list:
        ...
        if hit is not None:
            ...
            fallback = cam_origin + angle_to_direction(a_deg, p_deg) * max_range
    ...
    if out_path:
        ...
        print(f"3D scene saved: {out_path}")
    plt.show()
...
def compute_uv_for_triangle(mesh, face_idx, building_center):
    ...
    if np.dot(normal, face_center - building_center) < 0:
...
def export_textured_hits(mesh, hit_indices, output_folder, texture_path, building_center):
    if not hit_indices:
        print("No hit faces. Skipping export.")
        return False
    ...
    with open(output_obj, "w") as f:
        f.write(obj_data)
    print(f"Mesh exported to {output_obj}")
...
def get_wall_surfaces_info(gml_path):
    ...
    for wall in root.findall('.//bldg:WallSurface', ns):
        ...
        if all_x:
            ...
            z_min = min(all_z)
            z_max = max(all_z)
            surfaces.append({"cx": cx, "cy": cy, "z_min": z_min, "z_max": z_max, "z_mid": 0.5 * (z_min + z_max)})
    return surfaces
...
def main():
    csv_fov = "data/02_geofov/geofov_new_v3.csv"
    df_fov = load_fov_csv(csv_fov)
    df_fov = df_fov[df_fov["building_id"] == 4906972]
    mesh_path = "/home/tang/code/CityGML2OBJv2/output/DEBY_LOD2_4906972.obj"
    mesh = load_building_mesh(mesh_path)
    gml_path = "/home/tang/code/CityGML2OBJv2/data/tum2twin-datasets/citygml/lod2-building-datasets/DEBY_LOD2_4906972.gml"
    lower_bound, upper_bound = get_building_bounds(gml_path)
    if lower_bound is None or upper_bound is None:
        print("Failed to extract building height info from GML.")
        return
    print(f"Building lower bound: {lower_bound}, upper bound: {upper_bound}")
    surfaces = get_wall_surfaces_info(gml_path)
    print(f"Detected {len(surfaces)} WallSurface(s).")
    for i, surf in enumerate(surfaces):
        print(f"  WallSurface {i}: center=({surf['cx']:.2f}, {surf['cy']:.2f})")
    camera_z = lower_bound + 1.7
    offset_distance = 0.01
    texture_path = "/home/tang/code/ReLoD3_nus/raycasting/textured_mesh_4906972/4906972.png"
    ...
    for pid, group in grouped:
        ...
        print(f"\nCamera pid={pid}, cam_origin=({cam_x:.2f}, {cam_y:.2f}, {camera_z:.2f}), overall pitch={overall_pitch:.2f}°")
        best_dist = float('inf')
        ...
        pitch_samples = 5
        pitch_list = interpolate_pitch(best_pitch - 5.0, best_pitch + 5.0, pitch_samples)
        angle_samples = 10
        hits_list = []
        for _, row in group.iterrows():
            ...
            offset_angle = 3.0
            angles = interpolate_angles(left_a + offset_angle, right_a - offset_angle, angle_samples)
            for a in angles:
                for p in pitch_list:
                    hit_pt, tri_idx = do_raycast(mesh, cam_origin, a, p)
                    hits_list.append({"angle": a, "pitch": p, "hit": hit_pt, "tri": tri_idx})
        out_png = f"camera_pid_{pid}_raycasting.png"
        title = f"Camera pid={pid} (horizontal {angle_samples}, pitch {pitch_samples})"
        ...
        if hit_indices:
            ...
            output_folder = f"output_textured_mesh_4906972_{pid}"
            if export_textured_hits(mesh, hit_indices, output_folder, texture_path, building_center) and not exported:
                exported = True
        else:
            print(f"Camera pid={pid} had no valid ray hits. Skipping texture mapping.")
    if not exported:
        print("No cameras met texture mapping conditions. OBJ not exported.")
